				...
				if (retinaDashboard) {
					...
					jQuery.each(reply, function (index, sizes) {
						...
						jQuery.each(sizes, function (size, rsize) {
							if (rsize == 'EXISTS')
								jQuery('#wr2x_' + size + '_' + index).html("<img style='margin-top: 3px; width: 16px; height: 16px;' src='<?php echo trailingslashit( get_stylesheet_directory_uri() . '/plugins/' ) . trailingslashit( 'wp-retina-2x/img') . 'tick-circle.png'; ?>' />");
							else if (rsize == 'MISSING')
								jQuery('#wr2x_' + size + '_' + index).html("<img style='margin-top: 3px; width: 16px; height: 16px;' src='<?php echo trailingslashit( get_stylesheet_directory_uri() . '/plugins/' ) . trailingslashit( 'wp-retina-2x/img') . 'cross-small.png'; ?>' />");
							else if (rsize == 'PENDING')
								jQuery('#wr2x_' + size + '_' + index).html("<img style='margin-top: 3px; width: 16px; height: 16px;' src='<?php echo trailingslashit( get_stylesheet_directory_uri() . '/plugins/' ) . trailingslashit( 'wp-retina-2x/img') . 'clock.png'; ?>' />");
							else if (rsize == 'IGNORED')
								jQuery('#wr2x_' + size + '_' + index).html("<img style='margin-top: 3px; width: 16px; height: 16px;' src='<?php echo trailingslashit( get_stylesheet_directory_uri() . '/plugins/' ) . trailingslashit( 'wp-retina-2x/img') . 'prohibition-small.png'; ?>' />");
							else if (jQuery.isPlainObject(rsize))
								jQuery('#wr2x_' + size + '_' + index).html("<img title='Please upload a bigger original image.' style='margin-top: 3px; width: 16px; height: 16px;' src='<?php echo trailingslashit( get_stylesheet_directory_uri() . '/plugins/' ) . trailingslashit( 'wp-retina-2x/img') . 'exclamation.png'; ?>' /><span style='font-size: 9px; margin-left: 5px; position: relative; top: -4px;'><br />< " + rsize.width + "×" + rsize.height + "</span>");
							else {
					...
					if (response == 1) {
						// 1: SUCCESS
						jQuery('#wr2x_attachment_' + attachmentId).html("<img style='margin-top: -2px; margin-bottom: 2px;' src='<?php echo trailingslashit( get_stylesheet_directory_uri() . '/plugins/' ) . trailingslashit( 'wp-retina-2x/img') . 'tick-circle.png' ?>' />");
					}
					else {
						// 0: ERROR
